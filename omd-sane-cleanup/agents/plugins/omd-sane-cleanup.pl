...
# )
	...
	if ($type eq 'filesPerDay') {
		...
		$filesToSkip =
		  $main::conf->{'filesPerDay'} *
		  $main::conf->{'daysPerMonth'} *
		  $main::conf->{'MonthsPerYear'} *
		  $main::conf->{'maxYears'};
	} else {
	...
	for my $arrRef (@{$structuredFiles}) {
		...
		say STDERR '_clean_FilesByDate: ...sorted $structuredFiles[' . $i . ']: ' . Dumper($arrRef)
		  if $main::debug >= 1 and $i == 0;
